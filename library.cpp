...
using namespace std;
		...
		class Lib
			{
				public:
				char bookname[100],auname[50],BkID[20],BkID1[50];
				int q,B,p;
				Lib()
				{
					strcpy(bookname,"NO Book Name");
					strcpy(auname,"No Author Name");
					strcpy(BkID,"No Book ID");
					strcpy(BkID1,"No Book ID");
					q=0;
					B=0;
					p=0;
				}
		...
		void fine(int,int,int,int,int,int);
					...
					void Lib::getdata()
						...
						//entering book details into the system
						cout<<"\n\t\tEnter the details :-\n";
						cout<<"\n\t\tEnter Book's Name : ";
						...
						//this allows the user to insert the inputs asked
						cout<<"\n\t\tEnter Author's Name : ";
						cin.getline(auname,50);
						cout<<"\n\t\tEnter Publication name : ";
						cin.getline(BkID1,50);
						cout<<"\n\t\tEnter Book's ID : ";
						cin.getline(BkID,20);
						cout<<"\n\t\tEnter Book's Price : ";
						cin>>p;
						...
						cin>>q;
									...
									//method to show lib details
										void Lib::show(int i)
											{
												cout<<"\n\t\tBook Name : "<<bookname<<endl;
												cout<<"\n\t\tBook's Author Name : "<<auname<<endl;
												cout<<"\n\t\tBook's ID : "<<BkID<<endl;
												cout<<"\n\t\tBook's Publication : "<<BkID1<<endl;
												if(i==2)
													{
														cout<<"\n\t\tBook's Price : "<<p<<endl;
									...
									//method to get booklist from the system
									void Lib::booklist(int i)
									{
										...
										b=branch(i);
										system("cls");
										ifstream intf("BookDB.txt",ios::binary);
										if(!intf)
										cout<<"\n\t\tFile Not Found.";
									else
										{
										...
										while(!intf.eof())
										{
											if(b==B)
											{
												if(q==0 && i==1)
												{
													...
													//method to modify input into the system	
													void Lib::modify()
														{
															char ch,st1[100];
															int i=0,b,cont=0;
																		...
																		if(i==1)
																			{
																				system("cls");
																				b=branch(2);
																				ifstream intf1("BookDB.txt",ios::binary);
																				if(!intf1)
																					{
																						...
																						cin.getline(st1,100);
																						system("cls");
																						fstream intf("BookDB.txt",ios::in|ios::out|ios::ate|ios::binary);
																						intf.seekg(0);
																						intf.read((char*)this,sizeof(*this));
																						while(!intf.eof())
																						{
																						...
																						while(!intf.eof())
																						{
																						for(i=0;b==B&&BkID[i]!='\0'&&st1[i]!='\0'&&st1[i]==BkID[i];i++);
																						if(BkID[i]=='\0'&&st1[i]=='\0')
																						{
																						cont++;
																						getdata();
													...
													if(cont==0)
													...
													else if(i==2)
														{
															...
															system("cls");
															getdata();
															ofstream outf("BookDB.txt",ios::app|ios::binary);
											...
											//Types of books that are in the library system				
											cout<<"\n\t\t1.Programing\n\n\t\t2.Principles of accounting\n\n\t\t3.Communication Skills1\n\n\t\t4.Fundamentals of IT\n\n\t\t5.Principles of Management\n\n\t\t6.Fundamentals of Ecconomics\n\n\t\t7.Discrete Maths\n\n\t\t8.Go to main menu\n";
															...
															if(!intf)
																{
																	...
																	if(i==1)
																	...
																	else if(i==2)
																		{
																			...
																			while(!intf.eof())
																				{
																					for(i=0;b==B&&q!=0&&BkID[i]!='\0'&&ch[i]!='\0'&&ch[i]==BkID[i];i++);
																					if(BkID[i]=='\0'&&ch[i]=='\0')
																						{
																							cout<<"\n\t\tBook Found :-\n";
																							show(x);
											...
											//method to initiate the issuance of a book
											void Lib::issue()
												{
													char st[50],st1[20];
									...
									else if(i==3)
										{
											...
											while(!intf.eof())
												{
													for(i=0;BkID1[i]!='\0'&&st1[i]!='\0'&&st1[i]==BkID1[i];i++);
													if(BkID1[i]=='\0'&&st1[i]=='\0')
														{
															cont++;
															if(cont==1)
																{
									...
									else if(i==4)
										{
											...
											while(!intf.eof())
												{
													for(i=0;BkID[i]!='\0'&&st1[i]!='\0'&&st1[i]==BkID[i];i++);
													for(j=0;BkID1[j]!='\0'&&st[j]!='\0'&&st[j]==BkID1[j];j++);
													if(BkID[i]=='\0'&&BkID1[j]=='\0'&&st[j]=='\0'&&st1[i]=='\0')
														{
															d=q;
															m=B;
															y=p;
											...
											while(!intf.eof())
												{
													for(i=0;BkID[i]!='\0'&&st1[i]!='\0'&&st1[i]==BkID[i];i++);
													for(j=0;BkID1[j]!='\0'&&st[j]!='\0'&&st[j]==BkID1[j];j++);
													if(BkID[i]=='\0'&&BkID1[j]=='\0'&&st[j]=='\0'&&st1[i]=='\0'&&cont==0)
														{
															cont++;
															intf.read((char*)this,sizeof(*this));
															fine(q,B,p,d,m,y);
						...
						void Lib::der(char st[],int b,int x)
							{
								...
								while(!intf.eof())
									{
										for(i=0;b==B&&BkID[i]!='\0'&&st[i]!='\0'&&st[i]==BkID[i];i++);
										if(BkID[i]=='\0'&&st[i]=='\0')
										{
											cont++;
											if(x==1)
						...
						intf.close();
							...
							void Lib::get()
								{
									int i;
									cout<<"\n\t ____________________LEOS XXVII LIBRARY MANAGEMENT SYSTEM_____________________\n";
							...
							//Method to imput a password for the student	
								void Lib::studentpass()
									{
										int i=0;
										
										// the password for the library login is 12345
							...
							void Lib::student()
								{
									int i;
									cout<<"\n\t__________________ STUDENT _____________________\n";
									cout<<"\n\t\t>>Please Choose One Option:\n";
									cout<<"\n\t\t1.View Book List\n\n\t\t2.Search for a Book\n\n\t\t3.Go to main menu\n\n\t\t4.Close Application\n";
									cout<<"\n\t\tEnter your choice : ";
									cin>>i;
							...
							//Method to imput a password for the librarian	
								void Lib::pass()
									{
										int i=0;
								...
								if(st[i]=='\0'&&ch1[i]=='\0')
									...
									void Lib::librarian()
										{
											int i;
											cout<<"\n\t________________ LIBRARIAN______________________\n";
											cout<<"\n\t\t>>Please Choose One Option:\n";
											cout<<"\n\t\t1.View BookList\n\n\t\t2.Search for a Book\n\n\t\t3.Modify/Add Book\n\n\t\t4.Issue Book\n\n\t\t5.Go to main menu\n\n\t\t6.Change Password\n\n\t\t7.Close Application\n";
											cout<<"\n\t\tEnter your choice : ";
											cin>>i;
